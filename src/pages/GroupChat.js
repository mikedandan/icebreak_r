...
import { ChatFooter, ChatWindow } from '../components/ChatWindow'
...
export default function GroupChat() {
  ...
  const getChatHistory = async (position) => {
    ...
    try {
      console.log(
        `Location before sent to backend: \n ${position.lat},${position.lon}`,
      )
      const data = {
        namespace: 'group',
        lat: position.lat,
      ...
      const res = await axios.post(
        'https://icebreakr-serv.herokuapp.com/api/message/filterHistory',
        data,
      )
      return res.data
    } catch (err) {
  ...
  const postMessage = async (newMessage) => {
    return axios
      .post('https://icebreakr-serv.herokuapp.com/api/message/new', newMessage)
  ...
  return (
    <View style={styles.container} behavior="padding" enabled>
      ...
      {/* BackButton */}
      <Text>
        Lat: {positions.lat}Lon: {positions.lon}
      </Text>
      ...
      <View style={{ backgroundColor: '#E3E9EC', flex: 7 }}>
        <ChatWindow state={messages} currentUser={user} />
      </View>
